...
class GetRecentStarredCommand(Command):
    ...
    def onSelectedContact(self, match, rosanna):
        ...
        while len(messages) < self.args["count"] or self.args["count"] < 0:
            ...
            messageDiv = rosanna.findElement("./..", base=styled)
...
class SetMyNameCommand(Command):
    def execute(self, rosanna):
        ...
        wrapper = rosanna.findElement("//div[@id='app']/div/div/div/div")

        editButton = rosanna.findElement("./descendant::div[contains(@class,'copyable-text selectable-text')]/../../span/div", base=wrapper)
        rosanna.click(editButton)
        
        nameEl = rosanna.findElement("./descendant::div[contains(@class,'copyable-text selectable-text')]", base=wrapper)
        rosanna.clear(nameEl)
        rosanna.sendKeys(nameEl, self.args["name"])
        rosanna.sendKeys(nameEl, Keys.RETURN)

        backButton = rosanna.findElement("./span/div/div/header/div/div/button", base=wrapper)
        rosanna.click(backButton)

        wrappers = [None]
        while len(wrappers) > 0:
            wrappers = rosanna.driver.find_elements_by_xpath("//div[@id='app']/div/div/div/div/span/div/div/header/div/div[text()='Profile']")
...
class SetMyDescriptionCommand(Command):
    def execute(self, rosanna):
        ...
        editButton = rosanna.findElements("./descendant::div[contains(@class,'copyable-text selectable-text')]/../../span/div", base=wrapper)[1]
        rosanna.click(editButton)
        
        descEl = rosanna.findElements("./descendant::div[contains(@class,'copyable-text selectable-text')]", base=wrapper)[1]
...
class SetMyProfilePictureCommand(Command):
    def execute(self, rosanna):
        ...
        imageInput = rosanna.findElement("./descendant::input")
        rosanna.sendKeys(imageInput, self.args["path"])

        zoomOutButton = rosanna.findElement("//span[@data-icon='minus']/..")
        zoomInButton = rosanna.findElement("//span[@data-icon='plus']/..")

        zoomButton = zoomInButton if self.args["zoom"] > 0 else zoomOutButton
        for i in range(abs(self.args["zoom"])):
            rosanna.click(zoomButton)

        confirmButton = rosanna.findElement("//span[contains(@data-icon,'checkmark')]/..")
...
class GetContactDetailsCommand(Command):
    ...
    def onSelectedContact(self, match, rosanna):
        ...
        if header == "Contact info":
            ...
            profilePicEl, matchIndex = rosanna.findFirstElement(["./span/div/span/div/div/div/div/div/div/img[not(contains(@src,'dyn'))]",
                                                                 "./span/div/span/div/div/div/div/div/div/div/span[@data-icon='default-user']"],
                                                                base=wrapper)
            if matchIndex == 0:
            ...
            partCountSpan = rosanna.findElement("./span/div/span/div/div/div/div[position()=5]/div/div/div/div/span", base=wrapper)
            ...
            participantYs = []
            admins = []
            scrollable = rosanna.findElement("./span/div/span/div/div", base=wrapper)
                    ...
                    try:
                        ...
                        for participant in participantEls:
                            ...
                            if y not in participantYs:
                                ...
                                nameSpan, matchIndex = rosanna.findFirstElement(["./div/div/div[position()=2]/div/div/span/span",
                                                                                 "./div/div/div[position()=2]/div/div/span[@dir]"],
                                                                                base=participant)
            ...
            profilePicEl, matchIndex = rosanna.findFirstElement(["./descendant::div[contains(@style,'width: 200px; height: 200px;')]/img[not(contains(@src,'dyn'))]",
                                                                 "./descendant::span[@data-icon='default-group']"],
                                                                base=wrapper)
            if matchIndex == 0:
                ppSrc = profilePicEl.get_attribute("src")
...
class GetRecentContactsCommand(Command):
    def execute(self, rosanna):
        ...
        rosanna.findElement("//div[contains(@style, 'translateY')]")
        offset = 0
        while not finished:
            worked = False
            while not worked:
                try:
                    result = rosanna.findElement("//div[contains(@style, 'translateY(%ipx)')]" % (72*offset), timeout=0.01)
                    if result is None:
                        finished = True
                    else:
                        contactInteractable = ContactInteractable(rosanna, result)
...
class GetRecentMessagesCommand(Command):
    ...
    def onSelectedContact(self, match, rosanna):
        ...
        startTime = time.time()
        stopped = False
        window = rosanna.findElement("//div[@id='main']/div[position()=3]/div/div")
        messages = []
        while len(messages) < self.args["count"] or self.args["count"] < 0:
            rosanna.waitForLoading(base=window)
            divs = rosanna.findElements("./div[last()]/div", base=window)
            if len(divs) == 0:
                break
    ...
    def onSelectedContact(self, match, rosanna):
        ...
        if self.args["target"] == "Media":
            ...
            while len(wrappers) == 0:
                ...
                wrappers = rosanna.driver.find_elements_by_xpath("//div[@id='app']/div/span/div/div/div[position()=2]/div[position()=2]")
                print("try again")
            stopped = False
            while (len(messages) < self.args["count"] or self.args["count"] < 0) and not stopped:
                wrapper = rosanna.findElement("//div[@id='app']/div/span/div/div/div[position()=2]/div[position()=2]")
                ...
                previousButton = rosanna.findElement("./descendant::span[@title='Previous']", base=wrapper)
            ...
            while len(messages) < self.args["count"] or self.args["count"] < 0:
                ...
                div = divs[0]
                if len(divs) == 1:
                    parent = rosanna.findElement("./span/div/div/div", base=container)
                ...
                if not self.args["stripped"]:
                    platform = None
                    if self.args["target"] == "Links":
                        platform = rosanna.findElement("./descendant::span[contains(@class,'selectable-text invisible-space copyable-text')]", base=div)
                        rosanna.runScript("arguments[0].innerHTML='Processing...'", platform)
                    else:
                        platform = rosanna.findElement("./div/div/div/div/div", base=div)
                    rosanna.click(platform)
                    styled = rosanna.findElement(xpaths["velocityDiv"])
                    messageDiv = rosanna.findElement("./..", base=styled)
                    rosanna.runScript("arguments[0].removeAttribute('style')", styled)
...
class Rosanna(Thread):
    ...
    def downloadDocument(self, docEl, timeout=30):
        self.click(docEl)
        popup = self.findElement("//div[@id='app']/div/span/div/div/span", timeout=5)
        if popup is None:
        ...
        popups = self.driver.find_elements_by_xpath("//div[@id='app']/div/span/div/div/span")
        if len(popups) > 0:
    ...
    def loadMessageFromDiv(self, div, args):
        ...
        messageInEls = div.find_elements_by_xpath("./descendant::div[contains(@class, 'message-in')]")
        ...
        starred = len(div.find_elements_by_xpath("./descendant::span[@data-icon='star']")) == 1
        ...
        if len(contactEls) > 0:
            ...
            info["Name"] = contactWindow.find_element_by_xpath("./descendant::span[@dir='auto']").get_attribute("title")
            companies = contactWindow.find_elements_by_xpath("./div/div/div/div/div/div/div/div[position()=2]/div[position()=2]/div")
            if len(companies) > 0:
            ...
            if args["loadContactProfilePics"]:
                fakeProfilePics = contactWindow.find_elements_by_xpath("./descendant::img[contains(@src,'dyn')]")
                profilePics = contactWindow.find_elements_by_xpath("./descendant::img[not(contains(@src,'dyn'))]")
            ...
            for dataObject in dataObjects:
                value = self.getText(dataObject.find_element_by_xpath("./descendant::*[contains(@class,'selectable-text invisible-space copyable-text')]"))
                name = self.getText(dataObject.find_element_by_xpath("./div/div/div[position()=2]"))
                info[name] = value
    ...
    def loadMediaFromWrapper(self, wrapper, args):
        ...
        while (args["timeout"] < 0 or time.time()-startTime < args["timeout"]) and len(loadingEls) > 0:
            loadingEls = wrapper.find_elements_by_xpath("./descendant::span[@data-icon='media-disabled']")

        imgEls = wrapper.find_elements_by_xpath("./descendant::img")
        videoEls = wrapper.find_elements_by_xpath("./descendant::video[not(@loop)]")
        gifEls = wrapper.find_elements_by_xpath("./descendant::video[@loop]")
        audioEls = wrapper.find_elements_by_xpath("./descendant::audio")
        ...
        if len(blobEls) > 0:
            ...
            caption = None
            captionEls = wrapper.find_elements_by_xpath("./descendant::span[@dir='auto']")
            if len(captionEls) > 0:
                caption = self.getText(captionEls[0])
                    
            media = AttachmentClass(ext, data)
            starred = len(wrapper.find_elements_by_xpath("./descendant::span[@data-icon='unstar-btn']")) == 0
    ...
    def parseBlob(self, src, div):
        ...
        resultDiv = None
        while len(data) == 0:
            resultDivs = div.find_elements_by_xpath("./div[@id='blobResult']")
            if len(resultDivs) > 0:
...
class ContactInteractable:
    ...
    def read(self):
        return self.rosanna.findElement("./descendant::span[@title!='']", base=self.container).get_attribute("title")
